...
class lsu_trigger extends Module with RequireAsyncReset with lib {
 ...
 val trigger_enable = WireInit(0.U(1.W))
  ...
  val ldst_addr_trigger_m = io.lsu_addr_m & Fill(32, trigger_enable)
  val lsu_match_data = (0 until 4).map(i=>Mux1H(Seq(!io.trigger_pkt_any(i).select.asBool-> ldst_addr_trigger_m, (io.trigger_pkt_any(i).select & io.trigger_pkt_any(i).store).asBool->store_data_trigger_m)))

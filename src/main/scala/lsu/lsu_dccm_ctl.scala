...
class  lsu_dccm_ctl extends Module with RequireAsyncReset with lib
{
  val io = IO(new Bundle{
    ...
    val lsu_pkt_d            = Flipped(Valid(new lsu_pkt_t()))
  ...
  if (LOAD_TO_USE_PLUS1 == 1){
    ...
    //Registers
    lsu_rdata_corr_m       := Reverse(Cat(VecInit.tabulate(8)(i=> Reverse(Mux(((Cat(io.stbuf_fwdbyteen_hi_m,io.stbuf_fwdbyteen_lo_m))(i)).asBool,(Cat(io.stbuf_fwddata_hi_m,io.stbuf_fwddata_lo_m))((8*i)+7,8*i), Mux(io.addr_in_pic_m.asBool,picm_rd_data_m((8*i)+7,8*i),(Fill(2,io.addr_in_dccm_m) & dccm_rdata_corr_m((8*i)+7,8*i))))))))
  ...
  //DCCM inputs
  io.dccm.wren       := lsu_dccm_wren_d | io.lsu_stbuf_commit_any | io.ld_single_ecc_error_r_ff
  io.dccm.rden       := lsu_dccm_rden_d & io.addr_in_dccm_d

  io.dccm.wr_addr_lo := Mux(io.ld_single_ecc_error_r_ff.asBool,
  ...
  io.dccm.wr_addr_hi := Mux(io.ld_single_ecc_error_r_ff.asBool,
  ...
  io.dccm.rd_addr_lo := io.lsu_addr_d(DCCM_BITS-1,0)
  io.dccm.rd_addr_hi := io.end_addr_d(DCCM_BITS-1,0)

  io.dccm.wr_data_lo := Mux(io.ld_single_ecc_error_r_ff.asBool,
  ...
  io.dccm.wr_data_hi := Mux(io.ld_single_ecc_error_r_ff.asBool,
  ...
  io.dccm_rdata_lo_m      := io.dccm.rd_data_lo(DCCM_DATA_WIDTH-1,0)  //4 lines
  io.dccm_rdata_hi_m      := io.dccm.rd_data_hi(DCCM_DATA_WIDTH-1,0)
  io.dccm_data_ecc_lo_m   := io.dccm.rd_data_lo(DCCM_FDATA_WIDTH-1,DCCM_DATA_WIDTH)
  io.dccm_data_ecc_hi_m   := io.dccm.rd_data_hi(DCCM_FDATA_WIDTH-1,DCCM_DATA_WIDTH)

...
@chiselName
class el2_lsu_stbuf extends Module with param with RequireAsyncReset {
...
//  stbuf_addr := (0 until LSU_STBUF_DEPTH).map(i=> RegEnable(stbuf_addrin(i), 0.U, stbuf_wr_en(i).asBool())).reverse.reduce(Cat(_,_))
  stbuf_dma_kill := (0 until LSU_STBUF_DEPTH).map(i=> RegNext(Mux(stbuf_dma_kill_en(i).asBool,1.U ,stbuf_dma_kill(i)) & !stbuf_reset(i), 0.U)).reverse.reduce(Cat(_,_))

...
val csr=Module(new csr_tlu)
  ...
  io.tlu_busbuff <> csr.io.tlu_busbuff
...
//  rvdffe #(15)  mfdc_ff (.*, .en(wr_mfdc_r), .din({mfdc_ns[14:0]}), .dout(mfdc_int[14:0]));
 ...
 io.tlu_busbuff.dec_tlu_external_ldfwd_disable    := mfdc(11)
 io.dec_tlu_core_ecc_disable          := mfdc(8)
 io.tlu_busbuff.dec_tlu_sideeffect_posted_disable := mfdc(6)
 io.dec_tlu_bpred_disable             := mfdc(3)
 io.tlu_busbuff.dec_tlu_wb_coalescing_disable     := mfdc(2)
 ...
 mdseac_en := (io.tlu_busbuff.lsu_imprecise_error_store_any | io.tlu_busbuff.lsu_imprecise_error_load_any) & ~io.nmi_int_detected_f & ~io.mdseac_locked_f

 val mdseac = rvdffe(io.tlu_busbuff.lsu_imprecise_error_addr_any,mdseac_en.asBool,clock,io.scan_mode)
...
for(i <- 0 until 4) {io.trigger_pkt_any(i).tdata2 := mtdata2_t(i)}
 ...
 for(i <- 0 until 4) {
   mhpmc_inc_r(i) :=  (~mcountinhibit(i+3) & (Mux1H(Seq(
                   ...
                   (mhpme_vec(i) === MHPME_DBUS_TRANS      ).asBool -> io.tlu_busbuff.lsu_pmu_bus_trxn,
                   (mhpme_vec(i) === MHPME_DBUS_MA_TRANS   ).asBool -> io.tlu_busbuff.lsu_pmu_bus_misaligned,
                   (mhpme_vec(i) === MHPME_IBUS_ERROR      ).asBool -> io.ifu_pmu_bus_error,
                   (mhpme_vec(i) === MHPME_DBUS_ERROR      ).asBool -> io.tlu_busbuff.lsu_pmu_bus_error,
                   (mhpme_vec(i) === MHPME_IBUS_STALL      ).asBool -> io.ifu_pmu_bus_busy,
                   (mhpme_vec(i) === MHPME_DBUS_STALL      ).asBool -> io.tlu_busbuff.lsu_pmu_bus_busy,
